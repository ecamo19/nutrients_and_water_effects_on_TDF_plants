...
model_nodule_count <- readRDS("./processed_data/model_nodule_interaction.RData")
```
...
```{r figure_boxplot_nodules_spcode_by_treatment}
boxplot_plot_pmap(data = data_nodules_cleaned,
                                y = number_of_root_nodulation,
                                x = spcode,
                                fill = treatment)
```
...
poisson_nodule <- glm(number_of_root_nodulation ~ treatment*spcode + init_height ,
...
```{r}
car::Anova(poisson_nodule, type = "III")
```

## Tuckey comparisons

```{r}
as.data.frame(emmeans(poisson_nodule,
                           specs = pairwise ~ "treatment|spcode",
                           type = "response",
...
```{r}
eff_size(emmeans(gamma_nodule, ~treatment|spcode),
         sigma = sigma(gamma_nodule),
         edf = df.residual(gamma_nodule))
```
