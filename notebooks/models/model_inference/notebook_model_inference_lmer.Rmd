...
confidence interval of ±0.344 log-transformed fish. The back-transformed mean 
...
__Step done for backtransforming the emmeans__
```{r}
m1_amax_log <- lmer(log(amax) ~ treatment*nfixer + log(init_height) + (1|spcode), data = data_for_models)
```
```{r}
m1_gs_log <- lmer(log(gs) ~ treatment*nfixer + log(init_height) + (1|spcode), data = data_for_models)
```

```{r}
m1_wue_log <- lmer(log(wue) ~ treatment*nfixer + log(init_height) + (1|spcode), data = data_for_models)
```

```{r}
m1_pnue_log <- lmer(log(pnue) ~ treatment*nfixer + log(init_height) + (1|spcode), data = data_for_models)
```
...
models_q1_q2 %>% 
    ...
    purrr::list_modify("total_biomass" = NULL, "above_biomass"  = NULL, 
                       ...
                       # This trait did not reponded to the treatments this is
                       # why I excluded
                       "d13c" = NULL,
...
```{r message=FALSE, warning=FALSE}
tukey_table_tidy(models_q1_q2$pnue_log, single_model = T, formula = "treatment")
```
```{r}
emmeans(m1_pnue_log,
         pairwise ~ treatment,
...
```
```{r}
as.data.frame(emmeans(m1_gs_log,
                        pairwise ~ treatment|nfixer,
                        type = "response",
...
data_pred_traits <- 
        ...
        # Backtransform log vars
        mutate(
       
        pred_pnue = exp(pred_pnue_log),    
        pred_amax = exp(pred_amax_log),
        pred_gs = exp(pred_gs_log),
        pred_wue = exp(pred_wue_log)) %>% 
        # Remove original vars
        dplyr::select(c(1:3,15,17,21:24))  
```
...
```{r cache = TRUE}
three_way_param_bootstrap <- map_dfr(models_q3, ~bootstrap_model_df(., iter = 1000))
```
