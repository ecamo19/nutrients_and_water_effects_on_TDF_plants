...
## For double-ckeck
```{r}
tukey_rgr <- emmeans(models_q1$root_shoot_ratio_log,
                                ~treatment|nfixer, type = "response") %>%
                            pairs(adjust = "Tukey")
```


```{r}
eff_size(emmeans(models_q1$root_shoot_ratio_log,
                    ~treatment|nfixer), type = "response",
...
```{r}
emmeans(models_q1$total_biomass, ~treatment|nfixer) %>%
        pairs(adjust = "Tukey")
```
...
```{r}
emmeans(models_q1$rgr, ~treatment|nfixer) %>%
        pairs(adjust = "Tukey")
...
```{r}
emmeans_table_tidy(models_q1$root_shoot_ratio_log,  formula = "treatment|nfixer",
                    grouping_var = "nfixer")
```
...
https://www.youtube.com/watch?v=Te74FlbnkdE&t=331s