...
models_nodule_count 
    #models_nodule_count %>%
     ...
     #   purrr::list_modify( "glmer_poisson"  = NULL)
```
...
```{r}
sqrt(4.14)
arm::display(models_nodule_count$lmer_gaussian_log)
```


```{r}
report::report_parameters(models_nodule_count$lmer_gaussian_log)
help("standardize")
```

```{r}
# string <- c("models_nodule_count")
# 
# #data_pred_traits <- 
#         
#         # Get models prediction
#         gather_predictions(data_nodules_cleaned,
#                            
#                            # Return predictions
#                             models_nodule_count$lmer_gaussian) %>% 
#                        
#             
#             
#         pivot_wider(names_from = model, values_from = pred) %>%
...
#                 )) %>% 
#        
#         dplyr::select(c(1:3,ncol(.)))   %>% 
#             group_by(treatment) %>% 
#             summarise(mean(models_nodule_countlmer_gaussian))
```
