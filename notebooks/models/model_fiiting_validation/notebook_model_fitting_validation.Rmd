...
```{r}
lm_nodule_count <- lm(number_of_root_nodulation ~ treatment*spcode + init_height,
                       data = data_nodules_cleaned)
```


```{r}
lm_nodule_count_log <-  lm(log(number_of_root_nodulation) ~ treatment*spcode + init_height,
                           data = data_nodules_cleaned)
```
...
```{r}
glm_poisson <-  glm(number_of_root_nodulation ~ treatment*spcode + 
                    init_height, family = "poisson",data = data_nodules_cleaned)
```

```{r}
models_nodule_count <- list(glm_poisson, lm_nodule_count_log, lm_nodule_count)
names(models_nodule_count) <- c("glm_poisson","lm_nodule_count_log",
                                "lm_nodule_count")
```
