...
data_for_models <-
    data_for_models %>% 
        ...
        dplyr::select(!c(rgr_slope, d15n, agr))
```
...
response_vars_q1_q2 <- 
    set_names(names(data_for_models)[c(5:18)])
...
names(log_models) <- c("wue_log", "pnue_log", "rmf_log", 
                       "smf_log", "lmf_log", "ratio_log",
                       "nitrogen_log")
...
## Extra models Models: Question 3
...
```{r}
lme_3way_rmf <- lme(rmf ~  treatment:nfixer:amax +
                                            treatment:nfixer:gs  +
                                            treatment:nfixer:wue +
                                            treatment:nfixer:pnue +
                                            treatment:nfixer:d13c +
...
```{r}
lme_3way_smf <- lme(smf ~  treatment:nfixer:amax +
                                            treatment:nfixer:gs  +
                                            treatment:nfixer:wue +
                                            treatment:nfixer:pnue +
                                            treatment:nfixer:d13c +
...
```{r}
lme_3way_lmf <- lme(lmf ~  treatment:nfixer:amax +
                                            treatment:nfixer:gs  +
                                            treatment:nfixer:wue +
                                            treatment:nfixer:pnue +
                                            treatment:nfixer:d13c +
...
```{r}
nlme_validation_plots(lme_3way_rmf, data = data_for_models_scaled,
                      group = "spcode")
```

```{r}
nlme_validation_plots(lme_3way_smf, data = data_for_models_scaled,
                      group = "spcode")
```


```{r}
nlme_validation_plots(lme_3way_lmf, data = data_for_models_scaled,
                      group = "spcode")
```
...
models_3way_q3<- list(lme_above_biom_3way, lme_below_biom_3way,
                            ...
                            lme_3way_lmf, lme_3way_rmf, lme_3way_smf,
