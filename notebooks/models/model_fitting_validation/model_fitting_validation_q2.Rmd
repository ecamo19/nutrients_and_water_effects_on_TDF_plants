...
# Run model with spcies and treatment interaction
...
## Model
```{r}
linear_mod <- lm(number_of_root_nodulation ~ treatment * spcode + init_height,
                                    data = data_nodules_cleaned)
```


```{r}
check_model(linear_mod)
```

```{r}
car::Anova(linear_mod, type = "III")
```
