...
```{r}
lm_nodule_treatment_spcode <- lm(number_of_root_nodulation ~ treatment * spcode +
                                                              init_height,
...
```{r}
check_collinearity(lm_nodule_treatment_spcode )
```
...
```{r nudule_model_assumptions}
check_model(lm_nodule_treatment_spcode)
```
...
saveRDS(lm_nodule_treatment_spcode, file = "./processed_data/model_nodule_interaction.RData")
```