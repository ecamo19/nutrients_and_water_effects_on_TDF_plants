...
data_for_models <-
  ...
  dplyr::select(!c(rgr_slope, d15n, above_biomass, below_biomass, rmf, smf, lmf))
```
...
response_vars_q1 <-
  set_names(names(data_for_models)[c(5:8)])
...
## Root-Shoot ratio
model_q1_ratio_log <- lmer(log(root_shoot_ratio) ~ nfixer*treatment + init_height + (1|spcode),
                         data = data_for_models)

## Absolute growth rate
model_q1_agr_log <- lmer(log(agr) ~ nfixer*treatment + init_height + (1|spcode),
                         data = data_for_models)
```
...
log_models_q1 <- list(model_q1_ratio_log, model_q1_agr_log)

names(log_models_q1) <- c("root_shoot_ratio_log", "agr_log")
```
